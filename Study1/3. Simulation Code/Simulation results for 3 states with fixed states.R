#######################################################################################################################
# Author: 
# Purpose: Gets simulation results for application page with 3 states and fixed states
# 
# Note: 
...
## 1. Import data, parameters, functions ----
 ...
 # 3. Simulation: Loop over replicates and visitors -----
  ptime <- system.time({
      sim_1k_reps_3S_fixed <- foreach(icount(trials),.packages=c('expm','nnet', 'maotai', 'tidyverse')  ,.combine=rbind) %dopar% {
