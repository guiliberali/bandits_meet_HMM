#######################################################################################################################
# Author: 
# Purpose: Gets simulation results for application page with 3 states and HMM Bandit 
# 
# Note: 
...
## 1. Import data, parameters, functions ----
  ...
  MODEL    <- HMM_BANDIT # options are: HMM_BANDIT, MAB_FIXED_STATE, RANDOM
 ...
 # 3. Simulation: Loop over replicates and visitors -----
  ptime <- system.time({
      sim_1k_reps_3S_bandit <- foreach(icount(trials),.packages=c('expm','nnet', 'maotai', 'tidyverse')  ,.combine=rbind) %dopar% {
