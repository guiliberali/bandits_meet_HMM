...
## 1. Import data, parameters, functions ----
  ...
  MODEL    <- RANDOM # options are: HMM_BANDIT, MAB_FIXED_STATE, RANDOM
 ...
 # 3. Simulation: Loop over replicates and visitors -----
  ptime <- system.time({
      sim_1k_reps_3S_random <- foreach(icount(trials),.packages=c('expm','nnet', 'maotai', 'tidyverse')  ,.combine=rbind) %dopar% {
