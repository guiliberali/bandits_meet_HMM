...
if (TOT_STATES > 1 ) {mu_vec <-read.csv(paste0('../',FILENAME_MU), header=T) }
  
# Load HMM estimated parameters - rho_vec is ...?
if (TRANSITION_COVARIATES) rho_vec <- read.csv( paste0('../',FILENAME_RHO) , header=T)


# Load purchase probability 
temp_psm <- read.csv(paste0('../',FILENAME_PSM_per_state) , header=T)  
...
overall_success_rate_two_morphs = (sum(delta_sm_pre) + sum(delta_sm_post)) / TOT_VISITORS
