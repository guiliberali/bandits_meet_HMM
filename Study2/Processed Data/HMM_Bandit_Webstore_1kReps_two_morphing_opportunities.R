...
#  Code for two morphing opportunities, at t0 and at t7
#  May 12, 2021
## scp -r ~/Desktop/Online_store ubuntu@145.100.58.116:/data/morphing/ 
## scp -r ubuntu@145.100.58.116:/data/morphing/Online_store/results   ~/Documents/Algo_study_May2021/1k_sims_results 
 ...
 # Context:  "PHONE STORE" or "HULB2009"
  ...
  if (server)
  {
    PATH          <- "/data/morphing/Online_store/" 
    PATH_IN       <-  paste(PATH,"input",sep="")
  ...
  # Loop over replicates -----
   ...
   c(Success_rate, c(delta_sm_pre), c(delta_sm_post))
   }
  ...
  setwd(PATH_OUT)
  save.image("sim_hmm_bandit_store_pre_post_1kRun_100K_visitors.RData")
  

  ## compute summary stats on local----
  setwd("~/Documents/Algo_study_May2021/1k_sims_results/results" )
  load("sim_hmm_bandit_store_1kRun_pre_post_100K_visitors.RData")
   
  totReward = rowSums(sim_1krep[-1,])
   
  c(mean(totReward), 
     t.test(totReward)$conf.int)
  hist(totReward)
